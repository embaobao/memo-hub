...
import { IEmbedder, ICompleter, AIProviderRegistry } from '@memohub/ai-provider';
...
export class AIHub {
  ...
  public getEmbedder(agentId: string): IEmbedder {
    ...
    if (!provider) throw new Error(`Provider not found: ${agent.provider} for agent ${agentId}`);
