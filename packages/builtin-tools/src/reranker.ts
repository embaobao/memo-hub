...
export class RerankerTool implements ITool {
  ...
  public async execute(
    input: { query: string; results: any[]; agent: string; top_n: number },
...
${contextStr}`;
    ...
    try {
      const response = await completer.chat([
        {
          role: "system",
          content: "You are an expert at information retrieval and re-ranking.",
        },
      ...
      const reranked = ids
        .map((id: number) => input.results[id])
        .filter(Boolean);
