    ...
    try {
      ...
      if (options.category) {
        filteredRecords = allRecords.filter(
          (r: any) => r.category === options.category
        );
      }

      // 排序：按重要性降序
      filteredRecords.sort((a: any, b: any) => (b.importance || 0) - (a.importance || 0));
      ...
      if (records.length > 0) {
        ...
        for (let i = 0; i < records.length; i++) {
          ...
          console.log(
            `\n[${i + 1}] ${chalk.cyan(record.category)} ${chalk.yellow(
              `重要性: ${(record.importance || 0).toFixed(2)}`
            )}`
          );
...
program
  ...
  .action(async (options) => {
    ...
    try {
      ...
      if (options.language) {
        filteredRecords = allRecords.filter(
          (r: any) => r.language === options.language
        );
      }
      if (options.ast_type) {
        filteredRecords = allRecords.filter(
          (r: any) => r.ast_type === options.ast_type
        );
      }
      ...
      if (records.length > 0) {
        ...
        for (let i = 0; i < records.length; i++) {
          const record = records[i] as any;
          console.log(
            `\n[${i + 1}] ${chalk.cyan(record.language)} ${chalk.yellow(
              record.ast_type
            )} ${chalk.magenta(`重要性: ${(record.importance || 0).toFixed(2)}`)}`
          );
          console.log(chalk.gray(`ID: ${record.id}`));
          console.log(chalk.gray("-".repeat(80)));
  ...
  .action(async (category, options) => {
    ...
    try {
      ...
      for (let i = 0; i < filteredRecords.length; i++) {
        const record = filteredRecords[i] as any;
        const text = record.text || "";
  ...
  .action(async (options) => {
    ...
    try {
      ...
      if (options.category) {
        ...
        records = records.filter((r: any) => r.category === options.category);
      ...
      console.log(
        `  高重要性 (≥0.8): ${chalk.green(result.statistics.highImportance)}`
      );
      console.log(
        `  中等重要性 (0.6-0.8): ${chalk.yellow(
          result.statistics.mediumImportance
        )}`
      );
      console.log(
        `  低重要性 (<0.6): ${chalk.gray(result.statistics.lowImportance)}`
      );
  ...
  .action(async (options) => {
    ...
    try {
      ...
      result.extracted.forEach((item, index) => {
        console.log(
          `\n[${index + 1}] ${chalk.cyan(item.category)} ${chalk.yellow(
