...
**配置位置**: `~/.hermes/config.yaml` (第326-337行)
...
```bash
# 备份 v2 数据
cp -r ~/.hermes/data/gbrain.lancedb ~/.hermes/data/gbrain-backup-$(date +%Y%m%d)/
cp -r ~/.hermes/data/clawmem.lancedb ~/.hermes/data/clawmem-backup-$(date +%Y%m%d)/
