...
```bash
memohub clarification create "文档和实现对于查询入口描述不一致，需要用户确认" --agent hermes
```
...
```bash
memohub clarification resolve clarify_op_1 "当前以 UnifiedMemoryRuntime 为准" --agent hermes --project memo-hub --memory mem_old_note
```
