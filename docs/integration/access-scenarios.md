...
2. 启动和发现 MCP
   memohub serve -> resources/read memohub://tools
...
6. 写回资产
   ...
   memohub_resolve_clarification 写入用户澄清后的 curated memory
...
- `tools/list` 包含 `memohub_ingest_event`、`memohub_query`、`memohub_resolve_clarification`。
...
```bash
memohub clarify "文档和实现对于查询入口描述不一致，需要确认以哪个接口合同为准" --agent hermes
```

写回澄清：

```bash
memohub resolve-clarification clarify_op_1 "当前以 UnifiedMemoryRuntime、标准事件摄取和命名视图查询为准" --agent hermes --project memo-hub --memory mem_conflicting_note
```
...
- `resolve-clarification` 返回 `clarification.status=resolved`。
...
- 配置写入后 `memohub config` 能看到解析后的新值。
- `mcp-doctor.ok=true`。
- `mcp-logs` 能读取 NDJSON 日志。

## 场景 7：Agent Skill 安装后自动接入
...
8. 把用户澄清写入 memohub_resolve_clarification。
```
...
- Agent 后续使用 `memohub_ingest_event`、`memohub_query`、`memohub_resolve_clarification` 和配置工具完成接入闭环。
