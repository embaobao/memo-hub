...
- CLI：`memohub`
- MCP：`memohub mcp serve` 或 `memohub serve`
...
- [业务链路与治理规则](docs/architecture/business-workflows.md)
...
- MemoHub 只使用三种治理视角：`actor`、`project`、`global`。
- `actor` 视角用于主体自治理，例如 Hermes 查询和治理自己的记忆。
- `project` 视角用于同项目多主体混合上下文整合。
- `global` 视角用于全局资产治理、冲突检查、测试清理、归档恢复和审计。
- 跨 actor 聚合不得丢失原始归属、来源、时间和上下文绑定。

### 身份与挂载规则

- `actorId` 是记忆归属主体，也是默认治理主体。
- `agentId` 是执行实例或 profile，不是默认治理主入口。
- `channelId`、`sessionId` 属于运行挂载层，不是共享边界。
...
- 同一项目允许跨 actor 聚合查询和治理，但必须保留来源信息。
- 共享边界由 `scope` / `visibility` 控制，不由 channel 控制。
- 冲突记忆必须通过 clarification / resolution 闭环治理，不允许直接无痕覆盖。
