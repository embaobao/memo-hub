...
const supportedMimeTypes =
  defaultFileConfig.endpoints.default.supportedMimeTypes;

function makeEndpointConfig(
  overrides: Partial<EndpointFileConfig> = {},
): EndpointFileConfig {
  return {
    fileLimit: 10,
...
describe("validateFiles", () => {
  ...
  it("rejects when fileLimit would be exceeded", () => {
    ...
    files = new Map([
      [
        "f1",
        makeExtendedFile({ file_id: "f1", filename: "one.pdf", size: 2048 }),
      ],
      [
        "f2",
        makeExtendedFile({ file_id: "f2", filename: "two.pdf", size: 3072 }),
      ],
    ]);
    const fileList = [
  ...
  it("rejects unsupported MIME type", () => {
    ...
    expect(setError).toHaveBeenCalledWith(
      "Unsupported file type: application/x-unknown",
    );
  });
  ...
  it("rejects when file size equals fileSizeLimit (>= comparison)", () => {
    ...
    expect(result).toBe(false);
    expect(setError).toHaveBeenCalledWith(
      `File size limit exceeded: ${limit / megabyte} MB`,
    );
  });
  ...
  it("rejects when totalSizeLimit would be exceeded", () => {
    ...
    files = new Map([
      ["f1", makeExtendedFile({ file_id: "f1", size: 6 * megabyte })],
    ]);
    ...
    expect(result).toBe(false);
    expect(setError).toHaveBeenCalledWith(
      `Total file size limit exceeded: ${limit / megabyte} MB`,
    );
  });
  ...
  it("allows when totalSizeLimit is exactly met", () => {
    ...
    endpointFileConfig = makeEndpointConfig({ totalSizeLimit: limit });
    files = new Map([
      ["f1", makeExtendedFile({ file_id: "f1", size: 5 * megabyte })],
    ]);
  ...
  it("enforces check ordering: disabled before fileLimit", () => {
    ...
    files = new Map([
      ["f1", makeExtendedFile({ file_id: "f1", filename: "existing.pdf" })],
    ]);
    const fileList = [makeFile("doc.pdf", "application/pdf", 1024)];
    validateFiles({
      files,
