...
describe("Conversation Utilities", () => {
  describe("groupConversationsByDate", () => {
    ...
    it("skips conversations with duplicate conversationIds", () => {
      ...
      expect(grouped).toEqual(
        expect.arrayContaining([
          [
            " 2023",
            expect.arrayContaining([conversations[0], conversations[1]]),
          ],
    ...
    it("handles conversations from multiple years correctly", () => {
      ...
      expect(
        grouped[0][1].map((c) => new Date(c.updatedAt).getFullYear()),
      ).toEqual([2023, 2023]);
      expect(
        grouped[1][1].map((c) => new Date(c.updatedAt).getFullYear()),
      ).toEqual([2022]);
      expect(
        grouped[2][1].map((c) => new Date(c.updatedAt).getFullYear()),
      ).toEqual([2021, 2021]);
    });
  ...
  describe("InfiniteData helpers", () => {
    ...
    describe("updateInfiniteConvoPage", () => {
      it("updates a conversation in InfiniteData", () => {
        const data = {
          pages: [
            makePage([
              makeConversation("1", "2023-01-01T00:00:00Z"),
              makeConversation("2"),
            ]),
          ],
          pageParams: [],
        ...
        expect(updated?.pages[0].conversations[0].updatedAt).toBe(
      ...
      it("returns undefined if data is undefined", () => {
        expect(
          updateInfiniteConvoPage(undefined, "1", (c) => c),
        ).toBeUndefined();
      });
    });
    ...
    describe("addConversationToInfinitePages", () => {
      it("adds a conversation to the first page", () => {
        ...
        const updated = addConversationToInfinitePages(
          data,
          newConvo as TConversation,
        );
    ...
    describe("removeConvoFromInfinitePages", () => {
      it("removes a conversation by id", () => {
        ...
        expect(
          updated?.pages[0].conversations.map((c) => c.conversationId),
        ).toEqual(["1"]);
      });
    ...
    describe("QueryClient helpers", () => {
      ...
      it("addConvoToAllQueries adds new on top if not present", () => {
        ...
        const data = queryClient.getQueryData<InfiniteData<any>>([
          "allConversations",
        ]);
      ...
      it("addConvoToAllQueries does not duplicate", () => {
        ...
        expect(
          data!.pages[0].conversations.filter((c) => c.conversationId === "a")
            .length,
        ).toBe(1);
      });
      ...
      it("updateConvoInAllQueries with moveToTop when already at position 0 updates in place", () => {
        ...
        expect(data!.pages[0].conversations[0].updatedAt).not.toBe(
          originalUpdatedAt,
        );
      });

      it("updateConvoInAllQueries with moveToTop returns original data if convo not found", () => {
        const dataBefore = queryClient.getQueryData<InfiniteData<any>>([
          "allConversations",
        ]);
        updateConvoInAllQueries(
          queryClient,
        ...
        const dataAfter = queryClient.getQueryData<InfiniteData<any>>([
          "allConversations",
        ]);
      ...
      it("addConversationToAllConversationsQueries works with multiple pages", () => {
        ...
        const mainData = queryClient.getQueryData<InfiniteData<any>>([
          "allConversations",
        ]);
