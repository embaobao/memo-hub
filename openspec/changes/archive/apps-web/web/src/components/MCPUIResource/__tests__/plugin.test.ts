...
describe("mcpUIResourcePlugin", () => {
  ...
  describe("single resource markers", () => {
    it("should replace single UI resource marker with mcp-ui-resource node", () => {
      const tree = createTree([
        createTextNode(`Here is a resource ${UI_RESOURCE_MARKER}{abc123}`),
      ]);
      processTree(tree);
    ...
    it("should handle multiple single resource markers", () => {
      const tree = createTree([
        createTextNode(
          `First ${UI_RESOURCE_MARKER}{id1} and second ${UI_RESOURCE_MARKER}{id2}`,
        ),
      ]);
      processTree(tree);
    ...
    it("should handle hex IDs", () => {
      const tree = createTree([
        createTextNode(`Resource ${UI_RESOURCE_MARKER}{a3f2b8c1d4}`),
      ]);
      processTree(tree);
  ...
  describe("carousel markers", () => {
    it("should replace carousel marker with mcp-ui-carousel node", () => {
      const tree = createTree([
        createTextNode(`Carousel ${UI_RESOURCE_MARKER}{id1,id2,id3}`),
      ]);
      processTree(tree);
    ...
    it("should handle multiple IDs in carousel", () => {
      const tree = createTree([
        createTextNode(`${UI_RESOURCE_MARKER}{alpha,beta,gamma}`),
      ]);
      processTree(tree);

      const children = (tree as any).children;
      expect(children[0].data.hProperties.resourceIds).toEqual([
        "alpha",
        "beta",
        "gamma",
      ]);
  ...
  describe("id-based markers", () => {
    it("should replace single ID marker with mcp-ui-resource node", () => {
      const tree = createTree([
        createTextNode(`Check this ${UI_RESOURCE_MARKER}{abc123}`),
      ]);
      processTree(tree);
    ...
    it("should replace carousel ID marker with mcp-ui-carousel node", () => {
      const tree = createTree([
        createTextNode(`${UI_RESOURCE_MARKER}{one,two,three}`),
      ]);
      processTree(tree);
  ...
  describe("mixed content", () => {
    ...
    it("should handle marker at start of text", () => {
      const tree = createTree([
        createTextNode(`${UI_RESOURCE_MARKER}{id1} after`),
      ]);
      processTree(tree);
    ...
    it("should handle marker at end of text", () => {
      const tree = createTree([
        createTextNode(`Before ${UI_RESOURCE_MARKER}{id1}`),
      ]);
      processTree(tree);
