...
type SSEEventListener = (
  e: Partial<MessageEvent> & { responseCode?: number },
) => void;
...
interface MockSSEInstance {
  ...
  _emit: (
    event: string,
    data?: Partial<MessageEvent> & { responseCode?: number },
  ) => void;
}
...
jest.mock("~/data-provider", () => ({
  ...
  streamStatusQueryKey: (conversationId: string) => [
    "streamStatus",
    conversationId,
  ],
}));
...
const buildSubmission = (
  overrides: Partial<PartialSubmission> = {},
): TSubmission => {
...
describe("useResumableSSE - 404 error path", () => {
  ...
  it("clears the text and files draft from localStorage on 404", async () => {
    seedDraft(CONV_ID);
    expect(
      localStorage.getItem(`${LocalStorageKeys.TEXT_DRAFT}${CONV_ID}`),
    ).not.toBeNull();
    expect(
      localStorage.getItem(`${LocalStorageKeys.FILES_DRAFT}${CONV_ID}`),
    ).not.toBeNull();
