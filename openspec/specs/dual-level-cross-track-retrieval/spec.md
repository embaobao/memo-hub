# Capability: Dual-level Cross-track Retrieval

## Purpose
实现 Low-level 向量召回与 High-level 实体扩展的双层检索机制，支持跨轨道（GBrain/ClawMem）联动与结果溯源。
...
系统 MUST 使用高频实体在另一轨（或本轨）执行扩展召回，将包含 these 实体的记录加入候选集合，以实现跨轨联动。
