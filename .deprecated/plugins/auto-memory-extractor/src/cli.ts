 ...
 */
function loadSession(sessionPath: string): any | null {
  try {
 ...
 */
function listSessions(sessionsDir: string): string[] {
  if (!existsSync(sessionsDir)) {
    return []
  }

  const files = readdirSync(sessionsDir)
    .filter((f: string) => f.endsWith('.json') || f.endsWith('.ndjson'))
    .map((f: string) => join(sessionsDir, f))
