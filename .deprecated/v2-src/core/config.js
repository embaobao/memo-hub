...
export class ConfigManager {
    ...
    normalizeConfig(input) {
        ...
        const syncRaw = isPlainObject(raw.sync)
            ? raw.sync
            : {};
        const syncCloudRaw = isPlainObject(syncRaw.cloud) ? syncRaw.cloud : {};
        ...
        return {
            ...
            routing: {
                ...
                ...(routingRaw ?? {}),
                enabled: typeof routingRaw.enabled === "boolean"
                    ? routingRaw.enabled
                    : defaults.routing?.enabled ?? true,
                default_track: typeof routingRaw.default_track === "string" && String(routingRaw.default_track ?? "").trim() !== ""
    ...
    for (const item of value) {
        ...
        if (type === "file_suffix") {
            ...
            const suffixes = Array.isArray(item["suffixes"])
                ? item["suffixes"].map((s) => String(s ?? "").trim()).filter(Boolean)
                : [];
