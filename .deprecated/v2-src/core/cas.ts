...
export class ContentAddressableStorage {
   ...
   */
  async putText(text: string): Promise<CasPutResult> {
   ...
   * 读取原文
   */
  async getTextByRef(contentRef: string): Promise<string | null> {
   ...
   * 校验并读取原文（校验失败返回 null）
   */
  async getTextVerified(options: { contentRef: string; contentHash?: string }): Promise<string | null> {
