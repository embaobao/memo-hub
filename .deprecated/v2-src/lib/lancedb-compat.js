 ...
 */
async function tryGetTableFieldNames(table) {
    const schemaCandidate = table?.schema;
    if (!schemaCandidate) {
        return null;
    }
    let schema = schemaCandidate;
    try {
    ...
    const fields = (schema && Array.isArray(schema.fields) ? schema.fields : undefined) ??
...
function filterRowByAllowList(row, allowList) {
    const filtered = {};
 ...
 */
function extractUnknownFieldsFromError(error) {
 ...
 */
export async function addRowsWithSchemaCompatibility(table, rows, options) {
...
//# sourceMappingURL=lancedb-compat.js.map