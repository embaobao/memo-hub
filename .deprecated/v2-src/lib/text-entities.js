 ...
 */
export const DEFAULT_TEXT_ENTITY_EXTRACTOR_OPTIONS = {
    enabled: true,
    maxEntities: 24,
...
function clampMaxEntities(value, fallback) {
...
function isTokenLengthValid(token, minLength, maxLength) {
    const len = token.length;
    return len >= minLength && len <= maxLength;
}
function pushMatches(bag, source, regex, minLength, maxLength, shouldKeep) {
    regex.lastIndex = 0;
    let match;
 ...
 * - 去重后的 token 数组（按出现顺序）
 */
export function extractEntitiesFromText(text, options = {}) {
    const source = String(text ?? "");
    const enabled = options.enabled ?? DEFAULT_TEXT_ENTITY_EXTRACTOR_OPTIONS.enabled;
    if (!enabled) {
        return [];
    }
    const maxEntities = clampMaxEntities(options.maxEntities, DEFAULT_TEXT_ENTITY_EXTRACTOR_OPTIONS.maxEntities);
    if (maxEntities === 0) {
        return [];
    }
    const includeCamelCase = options.includeCamelCase ?? DEFAULT_TEXT_ENTITY_EXTRACTOR_OPTIONS.includeCamelCase;
    const includeDottedIdentifier = options.includeDottedIdentifier ??
    ...
    const minLength = clampMaxEntities(options.minLength, DEFAULT_TEXT_ENTITY_EXTRACTOR_OPTIONS.minLength);
    const maxLengthRaw = clampMaxEntities(options.maxLength, DEFAULT_TEXT_ENTITY_EXTRACTOR_OPTIONS.maxLength);
    ...
    if (includeVersion) {
         ...
         */
        const semverRegex = /\b(v?\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?(?:\+[0-9A-Za-z.-]+)?)\b/g;
