function normalizeSuffixList(suffixes) {
...
function createDefaultRule(options) {
...
function normalizeRuleConfigs(configs) {
 ...
 */
export function createRoutingEngine(routing) {
    ...
    const defaultTrack = typeof routing?.default_track === "string" && routing.default_track.trim() !== ""
        ? routing.default_track.trim()
        : "gbrain";
    const defaultCodeSuffixes = Array.isArray(routing?.code_suffixes) && routing.code_suffixes.length > 0
    ...
    if (rulesFromConfig && rulesFromConfig.length > 0) {
        for (const ruleConfig of rulesFromConfig) {
            if (ruleConfig.type === "file_suffix") {
                ...
                rules.push(createFileSuffixRule({
                    name: typeof ruleConfig.name === "string" && ruleConfig.name.trim() !== ""
                        ? ruleConfig.name.trim()
                        : "file_suffix",
                    track: typeof ruleConfig.track === "string" && ruleConfig.track.trim() !== ""
