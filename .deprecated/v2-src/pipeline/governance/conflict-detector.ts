import type {
  ...
  MemoConflictCandidate,
  MemoConflictPendingEvent,
  MemoContext,
  MemoRecord,
} from "../../types/index.js";
...
export function detectConflicts(options: {
  ...
  const {
    ...
    min_text_length = 12,
    max_candidates = 5,
  } = config ?? ({} as ConflictDetectionConfig);
  ...
  const scored: MemoConflictCandidate[] = [];
...
function buildReason(strategy: ConflictDetectionStrategy, score: number, threshold: number): string {
