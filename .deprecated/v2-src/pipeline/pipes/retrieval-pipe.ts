...
interface NormalizedRetrieveRequest extends MemoRetrieveRequest {
  query: string;
  tracks: string[];
  limit: number;
...
interface RetrievalWorkingSet {
  request: NormalizedRetrieveRequest;
  vector: MemoRecord[];
  fts: MemoRecord[];
}

interface AssembledResults {
  request: NormalizedRetrieveRequest;
  results: MemoRecord[];
}
...
function resolveTracks(input: MemoRetrieveRequest | undefined, registry: TrackRegistry): string[] {
  ...
  const filtersTrack = (filters as Record<string, unknown>)["track"];
  const filtersTracks = (filters as Record<string, unknown>)["tracks"];
  const fromFilters = [
...
function normalizeFtsEnabled(input: MemoRetrieveRequest | undefined): boolean {
  const { fts, filters = {} } = input ?? {};

  if (fts && typeof fts === "object" && typeof (fts as any).enabled === "boolean") {
    return Boolean((fts as any).enabled);
  }
   ...
   */
  const ftsEnabledFromFilters =
    typeof (filters as any)["fts_enabled"] === "boolean"
      ? (filters as any)["fts_enabled"]
      : typeof (filters as any)["ftsEnabled"] === "boolean"
        ? (filters as any)["ftsEnabled"]
        : undefined;
...
function recordKey(record: MemoRecord): string {
...
function mergeRecord(a: MemoRecord, b: MemoRecord): MemoRecord {
  /**
   * 多路合并策略（去重 + 补全）：
  ...
  return {
    ...primary,
    score: Number.isFinite(primary.score as number)
      ? (primary.score as number)
      : Number.isFinite(secondary.score as number)
        ? (secondary.score as number)
        : undefined,
    ...
    entities: Array.isArray(primary.entities) && primary.entities.length > 0
      ? primary.entities
      : secondary.entities,
    metadata: mergedMetadata as Record<string, unknown> | undefined,
  };
}

function mergeAndDedupeRecords(records: MemoRecord[]): MemoRecord[] {
...
function compareRecords(a: MemoRecord, b: MemoRecord): number {
...
export class RetrievalPipe {
  private pipe: BasePipe<MemoRetrieveRequest, MemoRecord[]>;

  constructor(options: RetrievalPipeOptions) {
    const { registry, eventBus, hydrator } = options;

    this.pipe = new BasePipe<MemoRetrieveRequest, MemoRecord[]>({
      name: "retrieval",
      eventBus,
    });

    const normalizeStage: PipelineStage<MemoRetrieveRequest, MemoRetrieveRequest> =
      {
        ...
        run: async (input) => {
          ...
          return {
            ...
            limit,
            filters: (filters && typeof filters === "object" ? filters : {}) as Record<string, unknown>,
            context,
            hydrate,
    ...
    const vectorRecallStage: PipelineStage<MemoRetrieveRequest, RetrievalWorkingSet> =
      {
        name: "vector_recall",
        run: async (input) => {
          const request = input as NormalizedRetrieveRequest;
    ...
    const ftsRecallStage: PipelineStage<RetrievalWorkingSet, RetrievalWorkingSet> =
      {
        name: "fts_recall",
        run: async (input) => {
          const { request, vector = [] } = input ?? {};
          const { tracks = [], limit = 5, fts } = request ?? ({} as NormalizedRetrieveRequest);
    ...
    const assembleStage: PipelineStage<RetrievalWorkingSet, AssembledResults> =
      {
        name: "assemble_results",
        run: async (input) => {
          const { request, vector = [], fts = [] } = input ?? {};
          const { limit = 5 } = request ?? ({} as NormalizedRetrieveRequest);
    ...
    const hydrateStage: PipelineStage<AssembledResults, MemoRecord[]> = {
      name: "hydrate_records",
      run: async (input) => {
        const { request, results } = input ?? ({} as AssembledResults);
        const { hydrate } = request ?? ({} as NormalizedRetrieveRequest);
  ...
  use<TInput, TOutput>(stage: PipelineStage<TInput, TOutput>) {
    this.pipe.use(stage);
    return this;
  }

  async run(request: MemoRetrieveRequest): Promise<MemoRecord[]> {
