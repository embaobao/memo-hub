...
export class IngestionPipe {
  private pipe: BasePipe<MemoIngestRequest, MemoRecord>;
  ...
  constructor(options: IngestionPipeOptions) {
    ...
    this.pipe = new BasePipe<MemoIngestRequest, MemoRecord>({
      name: "ingestion",
      eventBus,
    });

    const normalizeStage: PipelineStage<MemoIngestRequest, MemoIngestRequest> = {
      ...
      run: async (input, context) => {
        ...
        const mergedContext: MemoContext | undefined = requestContext ?? context;
        const normalizedTrack =
          typeof track === "string" && String(track ?? "").trim() !== "" ? String(track).trim() : undefined;
    ...
    const routeStage: PipelineStage<MemoIngestRequest, MemoIngestRequest> = {
      name: "route_track",
      run: async (input, context) => {
        ...
        if (typeof track === "string" && String(track ?? "").trim() !== "") {
          ...
          const existingPipeline =
            metadata && typeof metadata === "object" ? (metadata as Record<string, unknown>)["__pipeline"] : undefined;
          const pipelineMeta = (existingPipeline && typeof existingPipeline === "object"
            ? (existingPipeline as Record<string, unknown>)
    ...
    const casStage: PipelineStage<MemoIngestRequest, MemoIngestRequest> = {
      name: "persist_cas",
      run: async (input) => {
    ...
    const providerIngestStage: PipelineStage<MemoIngestRequest, MemoRecord> = {
  ...
  async run(request: MemoIngestRequest): Promise<MemoRecord> {
