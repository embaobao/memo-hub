...
export class GovernancePipe {
  private pipe: BasePipe<MemoGovernanceRequest, MemoGovernanceResult>;
  ...
  constructor(
    ...
    this.pipe = new BasePipe<MemoGovernanceRequest, MemoGovernanceResult>({
      name: "governance",
      eventBus,
    });
    ...
    const conflictDetectorStage: PipelineStage<MemoGovernanceRequest, MemoGovernanceResult> = {
  ...
  async run(request: MemoGovernanceRequest): Promise<MemoGovernanceResult> {
    const { context } = request ?? {};
    return this.pipe.run(request, context);
  }
}

function resolveConflictDetectionConfig(
  override?: Partial<ConflictDetectionConfig>
): ConflictDetectionConfig {
  /**
   * 默认队列路径：
  ...
  const resolved: ConflictDetectionConfig = {
    enabled: true,
    threshold: 0.9,
    strategy: "jaccard",
    min_text_length: 12,
