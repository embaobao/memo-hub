      ...
      dataService.updateConversation(payload),
    {
      onSuccess: (updatedConvo, payload) => {
        ...
        queryClient.setQueryData(
          [QueryKeys.conversation, targetId],
          updatedConvo,
        );
...
export const useTagConversationMutation = (
  ...
  t.TTagConversationResponse,
  unknown,
  t.TTagConversationRequest,
  unknown
> => {
  ...
  return useMutation(
    ...
    },
    {
      onSuccess: (_data: t.TSharedLinkResponse, vars, context) => {
        queryClient.setQueryData(
          [QueryKeys.sharedLinks, _data.conversationId],
          _data,
        );
    ...
    onError: (_err, _vars, context) => {
      if (context?.previousQueries) {
        context.previousQueries.forEach(
          (prevData: unknown, prevQueryKey: unknown) => {
...
export const useConversationTagMutation = ({
  ...
  t.TConversationTagResponse,
  unknown,
  t.TConversationTagRequest,
  unknown
> => {
  const queryClient = useQueryClient();
  const { onSuccess, ..._options } = options || {};
  const onMutationSuccess: typeof onSuccess = (_data, vars) => {
    queryClient.setQueryData<t.TConversationTag[]>(
      ...
      (queryData) => {
        ...
        if (tag === undefined || !tag.length) {
          ...
          const existingTagIndex = queryData.findIndex(
            (item) => item.tag === _data.tag,
          );
    ...
    if (
      vars.addToConversation === true &&
      vars.conversationId != null &&
      _data.tag
    ) {
      ...
      updateTagsInConversation(vars.conversationId, [
        ...(currentConvo.tags || []),
        _data.tag,
      ]);
    }
  ...
  const { updateTagsInConversation, replaceTagsInAllConversations } =
...
export const useDeleteTagInConversations = () => {
  ...
  const deleteTagInAllConversation = (deletedTag: string) => {
    ...
    for (let i = 0; i < conversationIdsWithTag.length; i++) {
      ...
      if (
        conversationData &&
        Array.isArray((conversationData as { tags?: string[] }).tags)
      ) {
  ...
  return useMutation(
    (payload: t.TDeleteConversationRequest) =>
      dataService.deleteConversation(
        payload,
      ) as Promise<t.TDeleteConversationResponse>,
    {
      onMutate: async () => {
...
export const useDuplicateConversationMutation = (
  ...
  t.TDuplicateConvoResponse,
  unknown,
  t.TDuplicateConvoRequest,
  unknown
> => {
  const queryClient = useQueryClient();
  const { onSuccess, ..._options } = options ?? {};
...
> => {
  ...
  return useMutation(
    (payload: t.TForkConvoRequest) => dataService.forkConversation(payload),
    {
      onSuccess: (data, vars, context) => {
        if (!vars.conversationId) {
          return;
...
export const useUploadConversationsMutation = (
  ...
  return useMutation<t.TImportResponse, unknown, FormData>({
    mutationFn: (formData: FormData) =>
      dataService.importConversationsFile(formData),
...
> => {
  return useMutation([MutationKeys.deletePreset], {
    mutationFn: (preset: t.TPreset | undefined) =>
      dataService.deletePreset(preset),
    ...(options || {}),
  });
};
...
> => {
  ...
  return useMutation(
    ({
      ...
      const version =
        endpointConfig?.version ?? defaultAssistantsVersion[endpoint];
              ...
              if (doc.assistant_id === variables.assistant_id) {
                return {
                  ...
                  append_current_datetime:
                    variables.data.append_current_datetime,
                };
              }
              return doc;
            });
...
export const useDeleteAssistantMutation = (
  ...
  return useMutation(
    ({ assistant_id, model, endpoint }: t.DeleteAssistantBody) => {
      ...
      const version =
        endpointsConfig?.[endpoint]?.version ??
    ...
    },
    {
      ...
      onSuccess: (_data, variables, context) => {
        ...
        const data = listRes.data.filter(
          (assistant) => assistant.id !== variables.assistant_id,
        );
...
> => {
  ...
  return useMutation([MutationKeys.updateAction], {
    mutationFn: (variables: t.UpdateActionVariables) =>
      dataService.updateAction(variables),
              ...
              if (assistant.id === variables.assistant_id) {
                return {
                  ...
                  tools: (assistant.tools ?? []).filter(
                    (tool) =>
                      !(tool.function?.name.includes(domain ?? "") ?? false),
                  ),
                };
              }
              return assistant;
...
export const useVerifyEmailMutation = (
  ...
  t.VerifyEmailResponse,
  unknown,
  t.TVerifyEmail,
  unknown
> => {
  return useMutation({
    mutationFn: (variables: t.TVerifyEmail) =>
      dataService.verifyEmail(variables),
    ...(options || {}),
  });
};
