...
export const useCreateAgentMutation = (
  ...
  return useMutation(
    (newAgentData: t.AgentCreateParams) =>
      dataService.createAgent(newAgentData),
    {
    ...
    },
    {
      ...
      onSuccess: (updatedAgent, variables, context) => {
        ...
        queryClient.setQueryData<t.Agent>(
          [QueryKeys.agent, variables.agent_id],
          updatedAgent,
        );
        queryClient.setQueryData<t.Agent>(
      ...
      onSuccess: (_data, variables, context) => {
        const data = ((keys: t.AgentListParams[]) => {
          ...
          keys.forEach((key) => {
            ...
            data = listRes.data.filter(
              (agent) => agent.id !== variables.agent_id,
            );
...
export const useDuplicateAgentMutation = (
  ...
  { agent: t.Agent; actions: t.Action[] },
  Error,
  ...
  >((params: t.DuplicateAgentBody) => dataService.duplicateAgent(params), {
    ...
    onSuccess: ({ agent, actions }, variables, context) => {
      ...
      const existingActions =
        queryClient.getQueryData<t.Action[]>([QueryKeys.actions]) || [];

      queryClient.setQueryData<t.Action[]>(
        [QueryKeys.actions],
        existingActions.concat(actions),
      );
      invalidateAgentMarketplaceQueries(queryClient);
...
> => {
  ...
  return useMutation<t.Agent, unknown, t.AgentAvatarVariables>({
    ...
    mutationFn: (variables: t.AgentAvatarVariables) =>
...
export const useRevertAgentVersionMutation = (
  ...
  t.Agent,
  Error,
  { agent_id: string; version_index: number }
> => {
  const queryClient = useQueryClient();
  return useMutation(
    ({
