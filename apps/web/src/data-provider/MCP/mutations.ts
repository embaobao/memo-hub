...
export const useCreateMCPServerMutation = (options?: {
  ...
  Error,
  t.MCPServerCreateParams
> => {
  const queryClient = useQueryClient();

  return useMutation(
    (data: t.MCPServerCreateParams) => dataService.createMCPServer(data),
    {
      onMutate: (variables) => options?.onMutate?.(variables),
      onError: (error, variables, context) =>
        options?.onError?.(error, variables, context),
      onSuccess: (newServer, variables, context) => {
        const listRes = queryClient.getQueryData<t.MCPServersListResponse>([
          QueryKeys.mcpServers,
        ]);
        if (listRes) {
          queryClient.setQueryData<t.MCPServersListResponse>(
            [QueryKeys.mcpServers],
            {
              ...listRes,
    ...
    }) => dataService.updateMCPServer(serverName, data),
    {
      ...
      onSuccess: (
        ...
        queryClient.setQueryData(
          [QueryKeys.mcpServer, variables.serverName],
          updatedServer,
        );
        queryClient.invalidateQueries([QueryKeys.mcpServers]);
        queryClient.invalidateQueries([QueryKeys.mcpTools]);
...
export const useDeleteMCPServerMutation = (options?: {
  ...
  return useMutation(
    (serverName: string) => dataService.deleteMCPServer(serverName),
    {
      onMutate: (variables) => options?.onMutate?.(variables),
      onError: (error, variables, context) =>
        options?.onError?.(error, variables, context),
