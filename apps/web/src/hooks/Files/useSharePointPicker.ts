...
export default function useSharePointPicker({
  ...
  const openSharePointPicker = async () => {
    if (!token) {
      showToast({
        message:
          "Unable to access SharePoint. Please ensure you are logged in with Microsoft.",
        status: "error",
      });
      return;
    }
    ...
    try {
      ...
      iframe.onload = () => {
        ...
        const url =
          sharePointBaseUrl + `/_layouts/15/FilePicker.aspx?${queryString}`;
  ...
  const activeEventListenerRef = useRef<((event: MessageEvent) => void) | null>(
    null,
  );
  ...
  const isAvailable =
    startupConfig?.sharePointFilePickerEnabled && isEntraIdUser && !tokenError;
