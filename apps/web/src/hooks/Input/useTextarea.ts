...
export default function useTextarea({
  ...
  const { index, conversation, isSubmitting, filesLoading, setFilesLoading } =
    useChatContext();
  const latestMessage = useRecoilValue(store.latestMessageFamily(index));
  const [activePrompt, setActivePrompt] = useRecoilState(
    store.activePromptByIndex(index),
  );
  ...
  useEffect(() => {
    ...
    const getPlaceholderText = () => {
      ...
      if (isAgent && (!currentAgentId || !agentsMap?.[currentAgentId])) {
        ...
        isAssistant &&
        (!currentAssistantId ||
          !assistantMap?.[currentEndpoint]?.[currentAssistantId])
      ) {
  ...
  const handleKeyDown = useCallback(
    (e: KeyEvent) => {
      ...
      if ((isNonShiftEnter || isCtrlEnter) && !isComposingInput) {
        const globalAudio = document.getElementById(globalAudioId) as
          | HTMLAudioElement
          | undefined;
        if (globalAudio) {
      ...
      if (clipboardData.files.length > 0) {
        ...
        for (const file of clipboardData.files) {
          const newFile = new File(
            [file],
            `clipboard_${+new Date()}_${file.name}`,
            {
              type: file.type,
            },
          );
