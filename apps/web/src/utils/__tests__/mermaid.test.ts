...
describe("mermaid config", () => {
  describe("flowchart config invariants", () => {
    ...
    it("both configs share the same base layout settings", () => {
      ...
      expect(inlineFlowchartConfig.nodeSpacing).toBe(
        artifactFlowchartConfig.nodeSpacing,
      );
      expect(inlineFlowchartConfig.rankSpacing).toBe(
        artifactFlowchartConfig.rankSpacing,
      );
      expect(inlineFlowchartConfig.padding).toBe(
        artifactFlowchartConfig.padding,
      );
    });
  });
  ...
  describe("getMermaidFiles", () => {
    ...
    it("produces dark theme files when isDarkMode is true", () => {
      ...
      expect(files["/components/ui/MermaidDiagram.tsx"]).toContain(
        'theme: "dark"',
      );
    ...
    it("does not inject custom themeVariables into generated component", () => {
      ...
      expect(darkFiles["/components/ui/MermaidDiagram.tsx"]).not.toContain(
        "themeVariables",
      );
      expect(lightFiles["/components/ui/MermaidDiagram.tsx"]).not.toContain(
        "themeVariables",
      );
    });
    ...
    it("handles empty content", () => {
      ...
      expect(files["diagram.mmd"]).toBe(
        "# No mermaid diagram content provided",
      );
    });
  });
  ...
  describe("fixSubgraphTitleContrast", () => {
    it("darkens title text on light subgraph backgrounds (fill attribute)", () => {
      ...
      expect(svg.querySelector("text")!.getAttribute("style")).toContain(
        "fill: #1a1a1a",
      );
    });
