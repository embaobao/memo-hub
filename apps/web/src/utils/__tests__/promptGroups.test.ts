...
describe("updateGroupFieldsInPlace", () => {
  it("updates matching group fields without changing its position", () => {
    ...
    expect(result.pages[0][InfiniteCollections.PROMPT_GROUPS][2]).toMatchObject(
      { _id: "c" },
    );
    // Length is preserved
  ...
  it("does NOT set updatedAt on the updated group", () => {
    ...
    expect(
      result.pages[0][InfiniteCollections.PROMPT_GROUPS][0].updatedAt,
    ).toBeUndefined();
  });
  ...
  it("handles an empty pages array without throwing", () => {
    const data = makeInfiniteData([]);
    expect(() =>
      updateGroupFieldsInPlace(data, { _id: "a", name: "Ghost" }),
    ).not.toThrow();
  });
