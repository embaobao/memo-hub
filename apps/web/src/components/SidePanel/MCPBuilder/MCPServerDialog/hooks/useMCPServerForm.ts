...
export function useMCPServerForm({
  ...
  const defaultValues = useMemo<MCPServerFormData>(() => {
    if (server) {
      ...
      const apiKeyConfig =
        "apiKey" in server.config ? server.config.apiKey : undefined;
      ...
      return {
        ...
        type:
          (server.config.type as "streamable-http" | "sse") ||
          "streamable-http",
    ...
    try {
      ...
      if (formData.auth.auth_type === AuthTypeEnum.ServiceHttp) {
        ...
        const authorizationType =
          formData.auth.api_key_authorization_type || "bearer";

        config.apiKey = {
          source,
          authorization_type: authorizationType,
          ...(source === "admin" &&
            formData.auth.api_key && { key: formData.auth.api_key }),
      ...
      if (error && typeof error === "object" && "response" in error) {
        ...
        if (axiosError.response?.data?.error === "MCP_INSPECTION_FAILED") {
          ...
          axiosError.response?.data?.error === "MCP_DOMAIN_NOT_ALLOWED"
        ) {
