...
const helpTextMap = {
  zh: {
    ...
    "Manage Hermes memory provider installation and diagnostics": "管理 Hermes 记忆提供者的安装与诊断",
    "Install the official MemoHub Hermes memory provider into Hermes user plugin directories": "将官方 MemoHub Hermes 记忆提供者安装到 Hermes 用户插件目录",
    "Hermes home directory": "Hermes 主目录",
    "Optional default project id written to provider config": "可选默认项目 ID，将写入 provider 配置",
    "Provider output language: auto, zh, or en": "Provider 输出语言：auto、zh 或 en",
    "Optional working directory written to provider config": "可选工作目录，将写入 provider 配置",
    "Check Hermes plugin links, provider config, Python compatibility, and plugin discoverability": "检查 Hermes 插件软链接、provider 配置、Python 兼容性与插件可发现性",
    "Remove Hermes plugin links without touching MemoHub memory data": "移除 Hermes 插件软链接，但不影响 MemoHub 记忆数据",
    "Also remove MemoHub-managed Hermes integration assets": "同时移除 MemoHub 托管的 Hermes 集成资产",
    ...
    "Install Hermes provider: memohub hermes install -> hermes memory setup -> hermes plugins reload": "安装 Hermes provider：memohub hermes install -> hermes memory setup -> hermes plugins reload",
    ...
    "Install official Hermes provider files first: memohub hermes install": "先安装官方 Hermes provider 文件：memohub hermes install",
    "Then activate it inside Hermes: hermes memory setup": "然后在 Hermes 内激活：hermes memory setup",
    "Reload Hermes plugins after updates: hermes plugins reload": "更新后重载 Hermes 插件：hermes plugins reload",
    "Verify links and Python compatibility: memohub hermes doctor": "检查软链接和 Python 兼容性：memohub hermes doctor",
...
export function localizeHelpOutput(text: string, lang: CliLang): string {
  ...
  output = output
    ...
    .replace(/Install official Hermes provider files first: memohub hermes install/gmu, "先安装官方 Hermes provider 文件：memohub hermes install")
    .replace(/Then activate it inside Hermes: hermes memory setup/gmu, "然后在 Hermes 内激活：hermes memory setup")
    .replace(/Reload Hermes plugins after updates: hermes plugins reload/gmu, "更新后重载 Hermes 插件：hermes plugins reload")
    .replace(/Verify links and Python compatibility: memohub hermes doctor/gmu, "检查软链接和 Python 兼容性：memohub hermes doctor")
    .replace(/Install Hermes provider: memohub hermes install -> hermes memory setup -> hermes plugins reload/gmu, "安装 Hermes provider：memohub hermes install -> hermes memory setup -> hermes plugins reload")
...
const messages = {
  zh: {
    ...
    contractTitle: "写入约定",
    guidanceTitle: "接入指引",
    hermesInstallTitle: "Hermes Provider 安装结果",
    hermesDoctorTitle: "Hermes Provider 诊断结果",
    hermesUninstallTitle: "Hermes Provider 卸载结果",
    ...
    reusable: "可复用",
    logPath: "日志路径",
    memohubRootLabel: "MemoHub 根目录",
    hermesHomeLabel: "Hermes 主目录",
    integrationPluginLabel: "集成插件目录",
    integrationConfigLabel: "集成配置文件",
    pluginLinkLabel: "Hermes 插件软链接",
    configLinkLabel: "Hermes 配置软链接",
    requiredPythonLabel: "最低 Python 要求",
    configVersion: "配置版本",
  ...
  en: {
    ...
    hermesInstallTitle: "Hermes Provider Install",
    hermesDoctorTitle: "Hermes Provider Doctor",
    hermesUninstallTitle: "Hermes Provider Uninstall",
    ...
    reusable: "Reusable",
    logPath: "Log path",
    memohubRootLabel: "MemoHub root",
    hermesHomeLabel: "Hermes home",
    integrationPluginLabel: "Integration plugin path",
    integrationConfigLabel: "Integration config path",
    pluginLinkLabel: "Hermes plugin link",
    configLinkLabel: "Hermes config link",
    requiredPythonLabel: "Required Python",
