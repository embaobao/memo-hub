...
program
  ...
  .action(async (text, options) => {
    ...
    try {
      ...
      if (result.success) {
        spinner.succeed(chalk.green(`Memory injected! [Trace: ${result.meta?.traceId}]`));
...
program
  ...
  .action(async () => {
    ...
    console.log(chalk.blue('AI Provider: ') + Object.keys(config.ai.providers).join(', '));
    console.log(chalk.blue('\nRegistered Tracks:'));
    tracks.forEach(t => console.log(` - ${chalk.green(t.id)} (${t.name || 'Unnamed'})`));
    console.log(chalk.blue('\nAtomic Tools:'));
    tools.forEach(t => console.log(` - ${chalk.magenta(t.id)}: ${t.description}`));
    console.log('');
  });
